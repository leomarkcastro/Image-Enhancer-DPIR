...
def sisr(source_folder: list, source_result: list, denoise=0 ,scale=[2], model_to_use="drunet_color"):

    # ----------------------------------------
    # Preparation
    # ----------------------------------------

    noise_level_img = denoise/255.0            # set AWGN noise level for LR image, default: 0, 
    noise_level_model = noise_level_img  # setnoise level of model, default 0
    model_name = model_to_use          # set denoiser, | 'drunet_color' | 'ircnn_gray' | 'drunet_gray' | 'ircnn_color'
    testset_name = source_folder[1]             # set test set,  'set5' | 'srbsd68'
    x8 = True                            # default: False, x8 to boost performance
    test_sf = scale                        # set scale factor, default: [2, 3, 4], [2], [3], [4]
