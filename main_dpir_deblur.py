...
def deblur(source_folder: list, source_result: list, denoise_level=7.65/255.0, model_to_use='drunet_color'):

    # ----------------------------------------
    # Preparation
    # ----------------------------------------

    noise_level_img = float(denoise_level)         # default: 0, noise level for LR image
    noise_level_model = noise_level_img  # noise level of model, default 0
    model_name = model_to_use           # 'drunet_gray' | 'drunet_color' | 'ircnn_gray' | 'ircnn_color'
    testset_name = source_folder[1]               # test set,  'set5' | 'srbsd68'
    ...
    model_path = os.path.join(model_zoo, "Copy of "+model_name+'.pth')
    ...
    iterations = range(kernels.shape[1])

    for k_index in iterations:
        ...
        for idx, img in enumerate(L_paths):
            ...
            if save_E or (_k_index+1 == kernels.shape[1]):
